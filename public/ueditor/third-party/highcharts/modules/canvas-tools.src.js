	...
	function build() {
		...
		svg.Property = function(name, value) {
			...
			this.Color = {
				...
				addOpacity: function(opacity) {
					...
					if (opacity != null && opacity != '') {
						let color = new RGBColor(that.value);
						if (color.ok) {
			...
			this.Definition = {
				...
				getDefinition: function() {
					let name = that.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2');
		...
		svg.Transform = function(v) {
			...
			this.Type.rotate = function(s) {
				...
				this.applyToPoint = function(p) {
					let a = this.angle.Angle.toRadians();
					p.applyTransform([1, 0, 0, 1, this.p.x || 0.0, this.p.y || 0.0]);
		...
		svg.Element.ElementBase = function(node) {
			...
			this.render = function(ctx) {
				...
				if (this.attribute('mask').hasValue()) {
					let mask = this.attribute('mask').Definition.getDefinition();
					if (mask != null) mask.apply(ctx, this);
				} else if (this.style('filter').hasValue()) {
					let filter = this.style('filter').Definition.getDefinition();
		...
		svg.Element.RenderedElementBase = function(node) {
			...
			this.setContext = function(ctx) {
				...
				if (this.attribute('clip-path').hasValue()) {
					let clip = this.attribute('clip-path').Definition.getDefinition();
		...
		svg.Element.rect = function(node) {
			...
			this.path = function(ctx) {
				let x = this.attribute('x').Length.toPixels('x');
				let y = this.attribute('y').Length.toPixels('y');
				let width = this.attribute('width').Length.toPixels('x');
				let height = this.attribute('height').Length.toPixels('y');
				let rx = this.attribute('rx').Length.toPixels('x');
				let ry = this.attribute('ry').Length.toPixels('y');
		...
		svg.Element.circle = function(node) {
			...
			this.path = function(ctx) {
				let cx = this.attribute('cx').Length.toPixels('x');
				let cy = this.attribute('cy').Length.toPixels('y');
				let r = this.attribute('r').Length.toPixels();
		...
		svg.Element.animate = function(node) {
			...
			this.calcValue = function() {
				let from = this.attribute('from').numValue();
				let to = this.attribute('to').numValue();
		...
		svg.Element.animateColor = function(node) {
			...
			this.calcValue = function() {
				let from = new RGBColor(this.attribute('from').value);
				let to = new RGBColor(this.attribute('to').value);

				if (from.ok && to.ok) {
					// tween color linearly
					let r = from.r + (to.r - from.r) * this.progress();
					let g = from.g + (to.g - from.g) * this.progress();
					let b = from.b + (to.b - from.b) * this.progress();
		...
		svg.Element.TextElementBase = function(node) {
			...
			this.renderChildren = function(ctx) {
				let customFont = this.parent.style('font-family').Definition.getDefinition();
				if (customFont != null) {
		...
		svg.Element.tref = function(node) {
			...
			this.getText = function() {
				let element = this.attribute('xlink:href').Definition.getDefinition();
...
(function (Highcharts) { // encapsulate
	...
	extend(CanVGRenderer.prototype, {
		...
		setContainer: function (container, chartWidth, chartHeight) {
			...
			let hiddenSvg = createElement(DIV, {
				width: containerOffsetWidth,
				height: containerOffsetHeight
			}, {
				visibility: HIDDEN,
