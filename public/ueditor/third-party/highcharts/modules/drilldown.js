(function(e){
	function q(b, a, c){
		...
		let c=b.series, d=c.xAxis, f=c.yAxis, e;e=b.color||c.color;var g, a=n({color: e}, a);g=HighchartsAdapter.inArray(this, c.points);this.drilldownLevels.push({seriesOptions: c.userOptions,
		...
		let b=this, a=this.getDrilldownBackText(), c=b.options.drilldown.drillUpButton;this.drillUpButton?this.drillUpButton.attr({text: a}).align():this.drillUpButton=this.renderer.button(a, null, null, function(){
			b.drillUp();
		}).attr(n({align: c.position.align, zIndex: 9}, c.theme)).add().align(c.position, !1, c.relativeTo||'plotBox');
	};h.prototype.drillUp=function(){
		let b=this.drilldownLevels.pop(), a=this.series[0],
	...
	k.prototype.animateDrilldown=function(b){
		let a=this.chart.drilldownLevels[this.chart.drilldownLevels.length-1], c=this.chart.options.drilldown.animation, d=a.shapeArgs, f=d.start, s=(d.end-f)/this.points.length, h=e.Color(a.color).rgba;b||g(this.points, function(a, b){
			let g=e.Color(a.color).rgba;a.graphic.attr(e.merge(d, {start: f+b*s, end: f+(b+1)*s})).animate(a.shapeArgs, e.merge(c, {step: function(a, d){
...
function(b){
	if(!b){
		let a=this, c=a.drilldownLevel;g(this.points, function(a){
			a.graphic.hide();a.dataLabel&&a.dataLabel.hide();a.connector&&a.connector.hide();
		});setTimeout(function(){
			g(a.points, function(a, b){
				let e=b===c.pointIndex?'show':'fadeIn';a.graphic[e]();if(a.dataLabel)a.dataLabel[e]();if(a.connector)a.connector[e]();
			});
		}, Math.max(this.chart.options.drilldown.animation.duration-50, 0));this.animate=m;
	}
};l.prototype.animateDrilldown=function(b){
		let a=this.chart.drilldownLevels[this.chart.drilldownLevels.length-
1].shapeArgs, c=this.chart.options.drilldown.animation;b||(a.x+=this.xAxis.oldPos-this.xAxis.pos, g(this.points, function(b){
		...
		let a=this.chart.options.drilldown.animation, c=this.group;delete this.group;g(this.points, function(d){
			let f=d.graphic, g=e.Color(d.color).rgba;delete d.graphic;f.animate(b.shapeArgs, e.merge(a, {step: function(a, c){
		...
		for(var b=this.series.chart, a=b.options.drilldown, c=a.series.length, d;c--&&!d;)a.series[c].id===this.drilldown&&(d=a.series[c]);r(b, 'drilldown', {point: this, seriesOptions: d});d&&b.addSeriesAsDrilldown(this, d);
		...
		var f=b.call(this, a, c, d), b=a.chart, a=(a=a.xAxis&&a.xAxis.ticks[d])&&a.label;if(f.drilldown){
			...
			a){
				if(!a._basicStyle)a._basicStyle=a.element.getAttribute('style');a.addClass('highcharts-drilldown-axis-label').css(b.options.drilldown.activeAxisLabelStyle).on('click', function(){
		...
		let a=this.chart.options.drilldown.activeDataLabelStyle;b.call(this);g(this.points, function(b){
