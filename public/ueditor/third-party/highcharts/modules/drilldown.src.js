...
(function (H) {
	...
	Chart.prototype.getDrilldownBackText = function () {
		let lastLevel = this.drilldownLevels[this.drilldownLevels.length - 1];
	...
	PieSeries.prototype.animateDrilldown = function (init) {
		let level = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1],
		...
		if (!init) {
			each(this.points, function (point, i) {
				let endColor = H.Color(point.color).rgba;
				...
				if (!init) {
					...
					setTimeout(function () {
						each(newSeries.points, function (point, i) {
							...
							let verb = i === level.pointIndex ? 'show' : 'fadeIn';
							point.graphic[verb]();
							if (point.dataLabel) {
	...
	ColumnSeries.prototype.animateDrilldown = function (init) {
		let animateFrom = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1].shapeArgs,
			animationOptions = this.chart.options.drilldown.animation;
	...
	function (level) {
		let animationOptions = this.chart.options.drilldown.animation,
			group = this.group;
	...
	wrap(H.Point.prototype, 'init', function (proceed, series, options, x) {
		let point = proceed.call(this, series, options, x),
			chart = series.chart,
	...
	wrap(H.Series.prototype, 'drawDataLabels', function (proceed) {
		let css = this.chart.options.drilldown.activeDataLabelStyle;
