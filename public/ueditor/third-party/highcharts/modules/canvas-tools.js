...
function RGBColor(m){
	this.ok=!1;m.charAt(0)=='#'&&(m=m.substr(1, 6));var m=m.replace(/ /g, ''), m=m.toLowerCase(), a={aliceblue: 'f0f8ff', antiquewhite: 'faebd7', aqua: '00ffff', aquamarine: '7fffd4', azure: 'f0ffff', beige: 'f5f5dc', bisque: 'ffe4c4', black: '000000', blanchedalmond: 'ffebcd', blue: '0000ff', blueviolet: '8a2be2', brown: 'a52a2a', burlywood: 'deb887', cadetblue: '5f9ea0', chartreuse: '7fff00', chocolate: 'd2691e', coral: 'ff7f50', cornflowerblue: '6495ed', cornsilk: 'fff8dc', crimson: 'dc143c', cyan: '00ffff', darkblue: '00008b',
			...
			slategray: '708090', snow: 'fffafa', springgreen: '00ff7f', steelblue: '4682b4', tan: 'd2b48c', teal: '008080', thistle: 'd8bfd8', tomato: 'ff6347', turquoise: '40e0d0', violet: 'ee82ee', violetred: 'd02090', wheat: 'f5deb3', white: 'ffffff', whitesmoke: 'f5f5f5', yellow: 'ffff00', yellowgreen: '9acd32'}, c;for(c in a)m==c&&(m=a[c]);let d=[{re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/, example: ['rgb(123, 234, 45)', 'rgb(255,234,245)'], process: function(b){
		...
		let b=d[c].process, k=d[c].re.exec(m);if(k)channels=b(k), this.r=channels[0], this.g=channels[1], this.b=channels[2], this.ok=!0;
	}this.r=this.r<0||isNaN(this.r)?0:this.r>255?255:this.r;this.g=this.g<0||isNaN(this.g)?0:
		this.g>255?255:this.g;this.b=this.b<0||isNaN(this.b)?0:this.b>255?255:this.b;this.toRGB=function(){
		return'rgb('+this.r+', '+this.g+', '+this.b+')';
	};this.toHex=function(){
		let b=this.r.toString(16), a=this.g.toString(16), d=this.b.toString(16);b.length==1&&(b='0'+b);a.length==1&&(a='0'+a);d.length==1&&(d='0'+d);return'#'+b+a+d;
	};this.getHelpXML=function(){
		for(var b=[], k=0;k<d.length;k++)for(var c=d[k].example, j=0;j<c.length;j++)b[b.length]=c[j];for(let h in a)b[b.length]=h;c=document.createElement('ul');
		c.setAttribute('id', 'rgbcolor-examples');for(k=0;k<b.length;k++)try{
			let l=document.createElement('li'), o=new RGBColor(b[k]), n=document.createElement('div');n.style.cssText='margin: 3px; border: 1px solid black; background:'+o.toHex()+'; color:'+o.toHex();n.appendChild(document.createTextNode('test'));let q=document.createTextNode(' '+b[k]+' -> '+o.toRGB()+' -> '+o.toHex());l.appendChild(n);l.appendChild(q);c.appendChild(l);
		}catch(p){}return c;
	};
}
if(!window.console)window.console={}, window.console.log=function(){}, window.console.dir=function(){};if(!Array.prototype.indexOf)Array.prototype.indexOf=function(m){
	for(let a=0;a<this.length;a++)if(this[a]==m)return a;return-1;
};
(function(){
	function m(){
		let a={FRAMERATE: 30, MAX_VIRTUAL_PIXELS: 3E4};a.init=function(c){
			...
			for(let c=0;c<a.Images.length;c++)if(!a.Images[c].loaded)return!1;return!0;
		};a.trim=function(a){
			return a.replace(/^\s+|\s+$/g, '');
		};a.compressSpaces=function(a){
			return a.replace(/[\s\r\t\n]+/gm, ' ');
		};a.ajax=function(a){
			let d;return(d=window.XMLHttpRequest?new XMLHttpRequest():new ActiveXObject('Microsoft.XMLHTTP'))?
			...
			if(window.DOMParser)return(new DOMParser()).parseFromString(a, 'text/xml');else{
				var a=a.replace(/<!DOCTYPE svg[^>]*>/, ''), d=new ActiveXObject('Microsoft.XMLDOM');d.async='false';d.loadXML(a);return d;
			}
		};a.Property=function(c, d){
			this.name=c;this.value=d;this.hasValue=function(){
				return this.value!=null&&this.value!=='';
			};this.numValue=function(){
				if(!this.hasValue())return 0;let b=parseFloat(this.value);(this.value+'').match(/%$/)&&
(b/=100);return b;
			...
			};let b=this;this.Color={addOpacity: function(d){
				let c=b.value;if(d!=null&&d!=''){
					let f=new RGBColor(b.value);f.ok&&(c='rgba('+f.r+', '+f.g+', '+f.b+', '+d+')');
				}return new a.Property(b.name, c);
			}};this.Definition={getDefinition: function(){
				let d=b.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2');return a.Definitions[d];
			}, isUrl: function(){
				return b.value.indexOf('url(')==
0;
			}, getFillStyle: function(b){
				let d=this.getDefinition();return d!=null&&d.createGradient?d.createGradient(a.ctx, b):d!=null&&d.createPattern?d.createPattern(a.ctx, b):null;
			}};this.Length={DPI: function(){
				return 96;
			}, EM: function(b){
				let d=12, c=new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize);c.hasValue()&&(d=c.Length.toPixels(b));return d;
			}, toPixels: function(d){
				if(!b.hasValue())return 0;let c=b.value+'';return c.match(/em$/)?b.numValue()*this.EM(d):c.match(/ex$/)?b.numValue()*this.EM(d)/2:c.match(/px$/)?
				...
				if(!b.hasValue())return 0;let a=b.value+'';if(a.match(/s$/))return b.numValue()*1E3;a.match(/ms$/);return b.numValue();
			}};this.Angle={toRadians: function(){
				if(!b.hasValue())return 0;let a=b.value+
...
g.fontFamily, fontSize: e||g.fontSize, fontStyle: d||g.fontStyle, fontWeight: c||g.fontWeight, fontVariant: b||g.fontVariant, toString: function(){
			...
			};let c=this;this.Parse=function(d){
				for(var b={}, d=a.trim(a.compressSpaces(d||'')).split(' '), k=!1, e=!1, f=!1, g=!1, j='', h=0;h<d.length;h++)if(!e&&c.Styles.indexOf(d[h])!=-1){
					if(d[h]!='inherit')b.fontStyle=d[h];e=!0;
				}else if(!g&&c.Variants.indexOf(d[h])!=-1){
					if(d[h]!='inherit')b.fontVariant=
d[h];e=g=!0;
				}else if(!f&&c.Weights.indexOf(d[h])!=-1){
					if(d[h]!='inherit')b.fontWeight=d[h];e=g=f=!0;
				}else if(k)d[h]!='inherit'&&(j+=d[h]);else{
					if(d[h]!='inherit')b.fontSize=d[h].split('/')[0];e=g=f=k=!0;
				}if(j!='')b.fontFamily=j;return b;
			};
		}();a.ToNumberArray=function(c){
			for(var c=a.trim(a.compressSpaces((c||'').replace(/,/g, ' '))).split(' '), d=0;d<c.length;d++)c[d]=parseFloat(c[d]);return c;
		};a.Point=function(a, d){
			this.x=a;this.y=d;this.angleTo=function(b){
				return Math.atan2(b.y-this.y, b.x-this.x);
			};this.applyTransform=
function(b){
	let a=this.x*b[1]+this.y*b[3]+b[5];this.x=this.x*b[0]+this.y*b[2]+b[4];this.y=a;
};
		};a.CreatePoint=function(c){
			c=a.ToNumberArray(c);return new a.Point(c[0], c[1]);
		};a.CreatePath=function(c){
			for(var c=a.ToNumberArray(c), d=[], b=0;b<c.length;b+=2)d.push(new a.Point(c[b], c[b+1]));return d;
		};a.BoundingBox=function(a, d, b, k){
			this.y2=this.x2=this.y1=this.x1=Number.NaN;this.x=function(){
				return this.x1;
			};this.y=function(){
				return this.y1;
			};this.width=function(){
...
this.y1;
				...
				if(b!=null){
					if(isNaN(this.x1)||isNaN(this.x2))this.x2=this.x1=b;if(b<this.x1)this.x1=b;if(b>this.x2)this.x2=b;
				}if(a!=null){
					if(isNaN(this.y1)||isNaN(this.y2))this.y2=this.y1=a;if(a<this.y1)this.y1=a;if(a>this.y2)this.y2=a;
				}
				...
				this.addPoint(b.x1, b.y1);this.addPoint(b.x2, b.y2);
...
a);this.addBezierCurve(b, a, d, d+1/3*(k-b), c, c+1/3*(l-a), k, l);
				...
				let q=[b, a], p=[d, c], t=[k, l], m=[o, n];this.addPoint(q[0], q[1]);this.addPoint(m[0], m[1]);for(i=0;i<=1;i++)b=function(b){
...
(-a+Math.sqrt(c))/(2*d), 0<k&&k<1&&(i==0&&this.addX(b(k)), i==1&&this.addY(b(k))), a=(-a-Math.sqrt(c))/(2*d), 0<a&&a<1&&(i==0&&this.addX(b(a)), i==1&&this.addY(b(a)))));
			...
			};this.addPoint(a, d);this.addPoint(b, k);
		};a.Transform=function(c){
			let d=this;this.Type={};this.Type.translate=function(b){
				this.p=a.CreatePoint(b);this.apply=function(b){
				...
				b=a.ToNumberArray(b);this.angle=new a.Property('angle', b[0]);this.cx=b[1]||0;this.cy=b[2]||0;this.apply=function(b){
					b.translate(this.cx, this.cy);b.rotate(this.angle.Angle.toRadians());b.translate(-this.cx, -this.cy);
				};this.applyToPoint=function(b){
					let a=this.angle.Angle.toRadians();b.applyTransform([1, 0, 0, 1, this.p.x||0, this.p.y||0]);b.applyTransform([Math.cos(a), Math.sin(a), -Math.sin(a), Math.cos(a), 0, 0]);b.applyTransform([1, 0, 0, 1, -this.p.x||
0, -this.p.y||0]);
				};
				...
				this.m=a.ToNumberArray(b);this.apply=function(b){
...
new a.Property('angle', b);
			};this.Type.SkewBase.prototype=new this.Type.matrix();this.Type.skewX=function(b){
				this.base=d.Type.SkewBase;this.base(b);this.m=[1, 0, Math.tan(this.angle.Angle.toRadians()), 1, 0, 0];
			};this.Type.skewX.prototype=new this.Type.SkewBase();this.Type.skewY=function(b){
				this.base=d.Type.SkewBase;this.base(b);this.m=[1, Math.tan(this.angle.Angle.toRadians()), 0, 1, 0, 0];
			};this.Type.skewY.prototype=new this.Type.SkewBase();this.transforms=[];this.apply=function(b){
				for(let a=0;a<this.transforms.length;a++)this.transforms[a].apply(b);
			};
			this.applyToPoint=function(b){
				for(let a=0;a<this.transforms.length;a++)this.transforms[a].applyToPoint(b);
			};for(var c=a.trim(a.compressSpaces(c)).split(/\s(?=[a-z])/), b=0;b<c.length;b++){
				let k=c[b].split('(')[0], e=c[b].split('(')[1].replace(')', '');this.transforms.push(new this.Type[k](e));
			}
...
p, f*=p);d=='slice'&&(k*=m, f*=m);h=new a.Property('refX', h);l=new a.Property('refY', l);h.hasValue()&&l.hasValue()?c.translate(-p*h.Length.toPixels('x'), -p*l.Length.toPixels('y')):(o.match(/^xMid/)&&(d=='meet'&&p==q||d=='slice'&&m==q)&&c.translate(b/2-k/2, 0), o.match(/YMid$/)&&(d=='meet'&&p==n||d=='slice'&&m==n)&&c.translate(0, e/2-f/2), o.match(/^xMax/)&&(d=='meet'&&p==q||d=='slice'&&m==q)&&c.translate(b-k, 0), o.match(/YMax$/)&&(d=='meet'&&p==n||d=='slice'&&m==n)&&c.translate(0, e-f));o=='none'?c.scale(n,
			...
			this.attributes={};this.styles={};this.children=[];this.attribute=function(b, d){
				let c=this.attributes[b];if(c!=null)return c;c=new a.Property(b, '');d==!0&&(this.attributes[b]=c);return c;
			};this.style=function(b, d){
				let c=this.styles[b];if(c!=null)return c;c=this.attribute(b);if(c!=null&&c.hasValue())return c;c=this.parent;if(c!=null&&(c=c.style(b), c!=null&&c.hasValue()))return c;
				...
				if(this.style('display').value!='none'&&this.attribute('visibility').value!='hidden'){
					b.save();this.setContext(b);if(this.attribute('mask').hasValue()){
						var a=this.attribute('mask').Definition.getDefinition();a!=null&&a.apply(b, this);
					}else this.style('filter').hasValue()?(a=this.style('filter').Definition.getDefinition(), a!=null&&a.apply(b, this)):this.renderChildren(b);this.clearContext(b);b.restore();
				}
			};this.setContext=
function(){};this.clearContext=function(){};this.renderChildren=function(b){
				for(let a=0;a<this.children.length;a++)this.children[a].render(b);
			};this.addChild=function(b, d){
				let c=b;d&&(c=a.CreateElement(b));c.parent=this;this.children.push(c);
			};if(c!=null&&c.nodeType==1){
				for(var d=0;d<c.childNodes.length;d++){
					var b=c.childNodes[d];b.nodeType==1&&this.addChild(b, !0);
				}for(d=0;d<c.attributes.length;d++)b=c.attributes[d], this.attributes[b.nodeName]=new a.Property(b.nodeName, b.nodeValue);b=a.Styles[c.nodeName];
				if(b!=null)for(var k in b)this.styles[k]=b[k];if(this.attribute('class').hasValue())for(var d=a.compressSpaces(this.attribute('class').value).split(' '), e=0;e<d.length;e++){
					b=a.Styles['.'+d[e]];if(b!=null)for(k in b)this.styles[k]=b[k];b=a.Styles[c.nodeName+'.'+d[e]];if(b!=null)for(k in b)this.styles[k]=b[k];
			...
			this.base=a.Element.ElementBase;this.base(c);this.setContext=function(d){
				if(this.style('fill').Definition.isUrl()){
					var b=this.style('fill').Definition.getFillStyle(this);if(b!=null)d.fillStyle=b;
...
this.style('stroke-linecap').value;if(this.style('stroke-linejoin').hasValue())d.lineJoin=this.style('stroke-linejoin').value;if(this.style('stroke-miterlimit').hasValue())d.miterLimit=this.style('stroke-miterlimit').value;if(typeof d.font!=='undefined')d.font=a.Font.CreateFont(this.style('font-style').value, this.style('font-variant').value, this.style('font-weight').value, this.style('font-size').hasValue()?this.style('font-size').Length.toPixels()+'px':'', this.style('font-family').value).toString();
			...
			this.base=a.Element.RenderedElementBase;this.base(c);this.path=function(d){
				...
				this.path(d);a.Mouse.checkPath(this, d);d.fillStyle!=''&&d.fill();d.strokeStyle!=''&&d.stroke();let b=this.getMarkers();if(b!=null){
					if(this.style('marker-start').Definition.isUrl()){
						var c=this.style('marker-start').Definition.getDefinition();c.render(d, b[0][0], b[0][1]);
					}if(this.style('marker-mid').Definition.isUrl())for(var c=this.style('marker-mid').Definition.getDefinition(), e=1;e<b.length-1;e++)c.render(d, b[e][0], b[e][1]);this.style('marker-end').Definition.isUrl()&&
(c=this.style('marker-end').Definition.getDefinition(), c.render(d, b[b.length-1][0], b[b.length-1][1]));
			...
			this.base=a.Element.RenderedElementBase;this.base(c);this.baseClearContext=this.clearContext;this.clearContext=function(d){
				this.baseClearContext(d);a.ViewPort.RemoveCurrent();
			...
			this.base=a.Element.PathElementBase;this.base(c);this.path=function(d){
				let b=this.attribute('x').Length.toPixels('x'), c=this.attribute('y').Length.toPixels('y'), e=this.attribute('width').Length.toPixels('x'), f=this.attribute('height').Length.toPixels('y'), g=this.attribute('rx').Length.toPixels('x'),
...
new a.Element.PathElementBase();a.Element.circle=function(c){
			this.base=a.Element.PathElementBase;this.base(c);this.path=function(d){
				let b=this.attribute('cx').Length.toPixels('x'), c=this.attribute('cy').Length.toPixels('y'), e=this.attribute('r').Length.toPixels();d!=null&&(d.beginPath(), d.arc(b, c, e, 0, Math.PI*2, !0), d.closePath());return new a.BoundingBox(b-e, c-e, b+e, c+e);
			};
		};a.Element.circle.prototype=new a.Element.PathElementBase();a.Element.ellipse=function(c){
			this.base=a.Element.PathElementBase;this.base(c);
			this.path=function(d){
				let b=4*((Math.sqrt(2)-1)/3), c=this.attribute('rx').Length.toPixels('x'), e=this.attribute('ry').Length.toPixels('y'), f=this.attribute('cx').Length.toPixels('x'), g=this.attribute('cy').Length.toPixels('y');d!=null&&(d.beginPath(), d.moveTo(f, g-e), d.bezierCurveTo(f+b*c, g-e, f+c, g-b*e, f+c, g), d.bezierCurveTo(f+c, g+b*e, f+b*c, g+e, f, g+e), d.bezierCurveTo(f-b*c, g+e, f-c, g+b*e, f-c, g), d.bezierCurveTo(f-c, g-b*e, f-b*c, g-e, f, g-e), d.closePath());return new a.BoundingBox(f-c, g-e, f+c, g+e);
			};
		};a.Element.ellipse.prototype=
new a.Element.PathElementBase();a.Element.line=function(c){
			this.base=a.Element.PathElementBase;this.base(c);this.getPoints=function(){
				...
				let b=this.getPoints();d!=null&&(d.beginPath(), d.moveTo(b[0].x, b[0].y), d.lineTo(b[1].x, b[1].y));return new a.BoundingBox(b[0].x, b[0].y, b[1].x, b[1].y);
			};
			this.getMarkers=function(){
				let a=this.getPoints(), b=a[0].angleTo(a[1]);return[[a[0], b], [a[1], b]];
			};
			...
			this.base=a.Element.PathElementBase;this.base(c);this.points=a.CreatePath(this.attribute('points').value);this.path=function(d){
				let b=new a.BoundingBox(this.points[0].x, this.points[0].y);d!=null&&(d.beginPath(), d.moveTo(this.points[0].x, this.points[0].y));for(let c=1;c<this.points.length;c++)b.addPoint(this.points[c].x,
					this.points[c].y), d!=null&&d.lineTo(this.points[c].x, this.points[c].y);return b;
			};this.getMarkers=function(){
				for(var a=[], b=0;b<this.points.length-1;b++)a.push([this.points[b], this.points[b].angleTo(this.points[b+1])]);a.push([this.points[this.points.length-1], a[a.length-1][1]]);return a;
			};
			...
			this.base=a.Element.polyline;this.base(c);this.basePath=this.path;this.path=function(a){
				let b=this.basePath(a);a!=null&&(a.lineTo(this.points[0].x,
			...
			this.base=a.Element.PathElementBase;this.base(c);c=this.attribute('d').value;c=c.replace(/,/gm, ' ');c=c.replace(/([MmZzLlHhVvCcSsQqTtAa])([MmZzLlHhVvCcSsQqTtAa])/gm, '$1 $2');c=c.replace(/([MmZzLlHhVvCcSsQqTtAa])([MmZzLlHhVvCcSsQqTtAa])/gm, '$1 $2');c=c.replace(/([MmZzLlHhVvCcSsQqTtAa])([^\s])/gm, '$1 $2');c=c.replace(/([^\s])([MmZzLlHhVvCcSsQqTtAa])/gm, '$1 $2');c=c.replace(/([0-9])([+\-])/gm,
				...
				this.tokens=d.split(' ');this.reset=function(){
					this.i=-1;this.previousCommand=this.command='';this.start=new a.Point(0, 0);this.control=new a.Point(0, 0);this.current=new a.Point(0, 0);this.points=[];this.angles=[];
				};this.isEnd=function(){
					...
					this.i+=1;return this.tokens[this.i];
					...
					let b=this.getPoint();return this.current=b;
					...
					if(this.isRelativeCommand())b.x=this.current.x+b.x, b.y=this.current.y+b.y;return b;
...
1]==null&&(this.angles[this.angles.length-1]=this.points[this.points.length-1].angleTo(d));this.addMarkerAngle(b, a==null?null:a.angleTo(b));
					...
					this.points.push(b);this.angles.push(a);
				};this.getMarkerPoints=function(){
					return this.points;
				};this.getMarkerAngles=function(){
					for(let b=0;b<this.angles.length;b++)if(this.angles[b]==null)for(let a=b+1;a<this.angles.length;a++)if(this.angles[a]!=null){
						this.angles[b]=this.angles[a];break;
					}return this.angles;
				};
			}(c);this.path=function(d){
				let b=
this.PathParser;b.reset();let c=new a.BoundingBox();for(d!=null&&d.beginPath();!b.isEnd();)switch(b.nextCommand(), b.command.toUpperCase()){
...
p.x-Math.sin(f)*p.y, (g.y+e.y)/2+Math.sin(f)*p.x+Math.cos(f)*p.y), m=function(b, a){
							...
							}, o=s([1, 0], [(n.x-p.x)/h, (n.y-p.y)/l]), q=[(n.x-p.x)/h, (n.y-p.y)/l], p=[(-n.x-p.x)/h, (-n.y-p.y)/l], n=s(q, p);if(m(q, p)<=-1)n=Math.PI;m(q, p)>=1&&(n=0);j==0&&n>0&&(n-=2*Math.PI);j==1&&n<0&&(n+=2*Math.PI);q=new a.Point(g.x-h*Math.cos((o+n)/
2), g.y-l*Math.sin((o+n)/2));b.addMarkerAngle(q, (o+n)/2+(j==0?1:-1)*Math.PI/2);b.addMarkerAngle(e, n+(j==0?1:-1)*Math.PI/2);c.addPoint(e.x, e.y);d!=null&&(m=h>l?h:l, e=h>l?1:h/l, h=h>l?l/h:1, d.translate(g.x, g.y), d.rotate(f), d.scale(e, h), d.arc(0, 0, m, o, o+n, 1-j), d.scale(1/e, 1/h), d.rotate(-f), d.translate(-g.x, -g.y));
			...
			this.base=a.Element.ElementBase;this.base(c);this.createPattern=function(d){
				let b=new a.Element.svg();b.attributes.viewBox=new a.Property('viewBox', this.attribute('viewBox').value);b.attributes.x=new a.Property('x', this.attribute('x').value);b.attributes.y=new a.Property('y', this.attribute('y').value);b.attributes.width=new a.Property('width', this.attribute('width').value);b.attributes.height=new a.Property('height',
					this.attribute('height').value);b.children=this.children;let c=document.createElement('canvas');c.width=this.attribute('width').Length.toPixels('x');c.height=this.attribute('height').Length.toPixels('y');b.render(c.getContext('2d'));return d.createPattern(c, 'repeat');
			};
...
'auto'&&d.rotate(c);this.attribute('markerUnits').valueOrDefault('strokeWidth')=='strokeWidth'&&d.scale(d.lineWidth, d.lineWidth);d.save();let e=new a.Element.svg();e.attributes.viewBox=new a.Property('viewBox', this.attribute('viewBox').value);e.attributes.refX=new a.Property('refX', this.attribute('refX').value);e.attributes.refY=new a.Property('refY', this.attribute('refY').value);e.attributes.width=new a.Property('width', this.attribute('markerWidth').value);e.attributes.height=new a.Property('height',
...
new a.Element.ElementBase();a.Element.defs=function(c){
			this.base=a.Element.ElementBase;this.base(c);this.render=function(){};
		};a.Element.defs.prototype=new a.Element.ElementBase();a.Element.GradientBase=function(c){
			this.base=a.Element.ElementBase;this.base(c);this.gradientUnits=this.attribute('gradientUnits').valueOrDefault('objectBoundingBox');this.stops=[];for(c=0;c<this.children.length;c++)this.stops.push(this.children[c]);this.getGradient=function(){};this.createGradient=function(d, b){
				let c=this;this.attribute('xlink:href').hasValue()&&
(c=this.attribute('xlink:href').Definition.getDefinition());for(var e=this.getGradient(d, b), f=0;f<c.stops.length;f++)e.addColorStop(c.stops[f].offset, c.stops[f].color);if(this.attribute('gradientTransform').hasValue()){
		...
		a.Element.GradientBase.prototype=new a.Element.ElementBase();a.Element.linearGradient=function(c){
			...
			this.base=a.Element.GradientBase;this.base(c);this.getGradient=function(a, b){
				let c=b.getBoundingBox(), e=this.gradientUnits=='objectBoundingBox'?
			...
			this.base=a.Element.ElementBase;this.base(c);a.Animations.push(this);this.duration=0;this.begin=this.attribute('begin').Time.toMilliseconds();this.maxDuration=this.begin+this.attribute('dur').Time.toMilliseconds();this.getProperty=function(){
				let a=this.attribute('attributeType').value, b=this.attribute('attributeName').value;return a=='CSS'?this.parent.style(b, !0):this.parent.attribute(b, !0);
			};this.initialValue=
...
(a=this.attribute('type').value+'('+a+')'), this.getProperty().value=a, a=!0;return a;
		...
		};a.Element.AnimateBase.prototype=new a.Element.ElementBase();a.Element.animate=function(c){
			this.base=a.Element.AnimateBase;this.base(c);this.calcValue=function(){
				let a=this.attribute('from').numValue(), b=this.attribute('to').numValue();return a+(b-a)*this.progress();
			};
		};a.Element.animate.prototype=new a.Element.AnimateBase();a.Element.animateColor=
function(c){
	this.base=a.Element.AnimateBase;this.base(c);this.calcValue=function(){
		var a=new RGBColor(this.attribute('from').value), b=new RGBColor(this.attribute('to').value);if(a.ok&&b.ok){
			var c=a.r+(b.r-a.r)*this.progress(), e=a.g+(b.g-a.g)*this.progress(), a=a.b+(b.b-a.b)*this.progress();return'rgb('+parseInt(c, 10)+','+parseInt(e, 10)+','+parseInt(a, 10)+')';
		}return this.attribute('from').value;
	};
};a.Element.animateColor.prototype=new a.Element.AnimateBase();a.Element.animateTransform=function(c){
			this.base=
a.Element.animate;this.base(c);
			...
			this.base=a.Element.ElementBase;this.base(c);this.horizAdvX=this.attribute('horiz-adv-x').numValue();this.isArabic=this.isRTL=!1;this.missingGlyph=this.fontFace=null;this.glyphs=[];for(c=0;c<this.children.length;c++){
				let d=this.children[c];if(d.type=='font-face')this.fontFace=d, d.style('font-family').hasValue()&&(a.Definitions[d.style('font-family').value]=this);else if(d.type=='missing-glyph')this.missingGlyph=
d;else if(d.type=='glyph')d.arabicForm!=''?(this.isArabic=this.isRTL=!0, typeof this.glyphs[d.unicode]==='undefined'&&(this.glyphs[d.unicode]=[]), this.glyphs[d.unicode][d.arabicForm]=d):this.glyphs[d.unicode]=d;
		...
		};a.Element.font.prototype=new a.Element.ElementBase();a.Element.fontface=function(c){
			this.base=a.Element.ElementBase;this.base(c);this.ascent=this.attribute('ascent').value;this.descent=this.attribute('descent').value;this.unitsPerEm=this.attribute('units-per-em').numValue();
		};a.Element.fontface.prototype=
new a.Element.ElementBase();a.Element.missingglyph=function(c){
			this.base=a.Element.path;this.base(c);this.horizAdvX=0;
		};a.Element.missingglyph.prototype=new a.Element.path();a.Element.glyph=function(c){
			this.base=a.Element.path;this.base(c);this.horizAdvX=this.attribute('horiz-adv-x').numValue();this.unicode=this.attribute('unicode').value;this.arabicForm=this.attribute('arabic-form').value;
		};a.Element.glyph.prototype=new a.Element.path();a.Element.text=function(c){
			this.base=a.Element.RenderedElementBase;
			this.base(c);if(c!=null){
				this.children=[];for(let d=0;d<c.childNodes.length;d++){
					let b=c.childNodes[d];b.nodeType==1?this.addChild(b, !0):b.nodeType==3&&this.addChild(new a.Element.tspan(b), !1);
				}
			}this.baseSetContext=this.setContext;this.setContext=function(b){
				this.baseSetContext(b);if(this.style('dominant-baseline').hasValue())b.textBaseline=this.style('dominant-baseline').value;if(this.style('alignment-baseline').hasValue())b.textBaseline=this.style('alignment-baseline').value;
			};this.renderChildren=
function(b){
	for(let a=this.style('text-anchor').valueOrDefault('start'), c=this.attribute('x').Length.toPixels('x'), d=this.attribute('y').Length.toPixels('y'), j=0;j<this.children.length;j++){
		let h=this.children[j];h.attribute('x').hasValue()?h.x=h.attribute('x').Length.toPixels('x'):(h.attribute('dx').hasValue()&&(c+=h.attribute('dx').Length.toPixels('x')), h.x=c);c=h.measureText(b);if(a!='start'&&(j==0||h.attribute('x').hasValue())){
			...
			this.base=a.Element.RenderedElementBase;this.base(c);this.getGlyph=function(a, b, c){
				let e=b[c], f=null;if(a.isArabic){
					let g='isolated';if((c==0||b[c-
1]==' ')&&c<b.length-2&&b[c+1]!=' ')g='terminal';c>0&&b[c-1]!=' '&&c<b.length-2&&b[c+1]!=' '&&(g='medial');if(c>0&&b[c-1]!=' '&&(c==b.length-1||b[c+1]==' '))g='initial';typeof a.glyphs[e]!=='undefined'&&(f=a.glyphs[e][g], f==null&&a.glyphs[e].type=='glyph'&&(f=a.glyphs[e]));
				...
				let b=this.parent.style('font-family').Definition.getDefinition();if(b!=null){
					let k=this.parent.style('font-size').numValueOrDefault(a.Font.Parse(a.ctx.font).fontSize),
						e=this.parent.style('font-style').valueOrDefault(a.Font.Parse(a.ctx.font).fontStyle), f=this.getText();b.isRTL&&(f=f.split('').reverse().join(''));for(let g=a.ToNumberArray(this.parent.attribute('dx').value), j=0;j<f.length;j++){
						let h=this.getGlyph(b, f, j), l=k/b.fontFace.unitsPerEm;c.translate(this.x, this.y);c.scale(l, -l);let o=c.lineWidth;c.lineWidth=c.lineWidth*b.fontFace.unitsPerEm/k;e=='italic'&&c.transform(1, 0, 0.4, 1, 0, 0);h.render(c);e=='italic'&&c.transform(1, 0, -0.4, 1, 0, 0);c.lineWidth=o;c.scale(1/
l, -1/l);c.translate(-this.x, -this.y);this.x+=k*(h.horizAdvX||b.horizAdvX)/b.fontFace.unitsPerEm;typeof g[j]!=='undefined'&&!isNaN(g[j])&&(this.x+=g[j]);
				...
				let b=this.parent.style('font-family').Definition.getDefinition();if(b!=null){
					var c=this.parent.style('font-size').numValueOrDefault(a.Font.Parse(a.ctx.font).fontSize),
						k=0, e=this.getText();b.isRTL&&(e=e.split('').reverse().join(''));for(let f=a.ToNumberArray(this.parent.attribute('dx').value), g=0;g<e.length;g++){
						let j=this.getGlyph(b, e, g);k+=(j.horizAdvX||b.horizAdvX)*c/b.fontFace.unitsPerEm;typeof f[g]!=='undefined'&&!isNaN(f[g])&&(k+=f[g]);
					}return k;
		...
		a.Element.tspan=function(c){
			this.base=a.Element.TextElementBase;this.base(c);this.text=c.nodeType==3?c.nodeValue:c.childNodes.length>0?c.childNodes[0].nodeValue:c.text;this.getText=function(){
				return this.text;
			};
		};a.Element.tspan.prototype=new a.Element.TextElementBase();a.Element.tref=function(c){
			this.base=a.Element.TextElementBase;this.base(c);this.getText=function(){
				let a=this.attribute('xlink:href').Definition.getDefinition();if(a!=null)return a.children[0].getText();
			};
		};a.Element.tref.prototype=new a.Element.TextElementBase();
		a.Element.a=function(c){
			this.base=a.Element.TextElementBase;this.base(c);this.hasText=!0;for(let d=0;d<c.childNodes.length;d++)if(c.childNodes[d].nodeType!=3)this.hasText=!1;this.text=this.hasText?c.childNodes[0].nodeValue:'';this.getText=function(){
				return this.text;
			...
			this.base=a.Element.RenderedElementBase;this.base(c);a.Images.push(this);this.img=document.createElement('img');this.loaded=!1;let d=this;this.img.onload=
function(){
	d.loaded=!0;
};this.img.src=this.attribute('xlink:href').value;this.renderChildren=function(b){
				let c=this.attribute('x').Length.toPixels('x'), d=this.attribute('y').Length.toPixels('y'), f=this.attribute('width').Length.toPixels('x'), g=this.attribute('height').Length.toPixels('y');f==0||g==0||(b.save(), b.translate(c, d), a.AspectRatio(b, this.attribute('preserveAspectRatio').value, f, this.img.width, g, this.img.height, 0, 0), b.drawImage(this.img, 0, 0), b.restore());
			};
		};a.Element.image.prototype=new a.Element.RenderedElementBase();
		a.Element.g=function(c){
			this.base=a.Element.RenderedElementBase;this.base(c);this.getBoundingBox=function(){
				for(var c=new a.BoundingBox(), b=0;b<this.children.length;b++)c.addBoundingBox(this.children[b].getBoundingBox());return c;
			};
		};a.Element.g.prototype=new a.Element.RenderedElementBase();a.Element.symbol=function(c){
...
a.ToNumberArray(this.attribute('viewBox').value), k=b[0], e=b[1];width=b[2];height=b[3];a.AspectRatio(c, this.attribute('preserveAspectRatio').value, this.attribute('width').Length.toPixels('x'), width, this.attribute('height').Length.toPixels('y'), height, k, e);a.ViewPort.SetCurrent(b[2], b[3]);
			...
			this.base=a.Element.ElementBase;this.base(c);for(var c=c.childNodes[0].nodeValue+(c.childNodes.length>1?c.childNodes[1].nodeValue:
				...
				let f=a.trim(k[e]);if(f!=''){
					for(var g={}, j=0;j<b.length;j++){
						var h=b[j].indexOf(':'), l=b[j].substr(0, h), h=b[j].substr(h+1, b[j].length-h);l!=null&&h!=null&&(g[a.trim(l)]=new a.Property(a.trim(l), a.trim(h)));
					}a.Styles[f]=g;if(f=='@font-face'){
						f=g['font-family'].value.replace(/"/g,
							'');g=g.src.value.split(',');for(j=0;j<g.length;j++)if(g[j].indexOf('format("svg")')>0){
							l=g[j].indexOf('url');h=g[j].indexOf(')', l);l=g[j].substr(l+5, h-l-6);l=a.parseXml(a.ajax(l)).getElementsByTagName('font');for(h=0;h<l.length;h++){
								let o=a.CreateElement(l[h]);a.Definitions[f]=o;
							}
						}
					}
				}
			...
			this.base=a.Element.RenderedElementBase;this.base(c);this.baseSetContext=this.setContext;this.setContext=function(a){
				...
				let a=this.attribute('xlink:href').Definition.getDefinition();if(this.attribute('width').hasValue())a.attribute('width', !0).value=this.attribute('width').value;if(this.attribute('height').hasValue())a.attribute('height', !0).value=this.attribute('height').value;return a;
			};this.path=function(a){
				let b=
this.getDefinition();b!=null&&b.path(a);
			};this.renderChildren=function(a){
				let b=this.getDefinition();b!=null&&b.render(a);
			};
			...
			this.base=a.Element.ElementBase;this.base(c);this.apply=function(a, b){
				let c=this.attribute('x').Length.toPixels('x'), e=this.attribute('y').Length.toPixels('y'), f=this.attribute('width').Length.toPixels('x'), g=this.attribute('height').Length.toPixels('y'), j=b.attribute('mask').value;b.attribute('mask').value=
'';let h=document.createElement('canvas');h.width=c+f;h.height=e+g;let l=h.getContext('2d');this.renderChildren(l);let o=document.createElement('canvas');o.width=c+f;o.height=e+g;let n=o.getContext('2d');b.render(n);n.globalCompositeOperation='destination-in';n.fillStyle=l.createPattern(h, 'no-repeat');n.fillRect(0, 0, c+f, e+g);a.fillStyle=n.createPattern(o, 'no-repeat');a.fillRect(0, 0, c+f, e+g);b.attribute('mask').value=j;
			};this.render=function(){};
		};a.Element.mask.prototype=new a.Element.ElementBase();a.Element.clipPath=
function(c){
	this.base=a.Element.ElementBase;this.base(c);this.apply=function(a){
		for(let b=0;b<this.children.length;b++)this.children[b].path&&(this.children[b].path(a), a.clip());
	};this.render=function(){};
};a.Element.clipPath.prototype=new a.Element.ElementBase();a.Element.filter=function(c){
			this.base=a.Element.ElementBase;this.base(c);this.apply=function(a, b){
				let c=b.getBoundingBox(), e=this.attribute('x').Length.toPixels('x'), f=this.attribute('y').Length.toPixels('y');if(e==0||f==0)e=c.x1, f=c.y1;let g=
this.attribute('width').Length.toPixels('x'), j=this.attribute('height').Length.toPixels('y');if(g==0||j==0)g=c.width(), j=c.height();c=b.style('filter').value;b.style('filter').value='';let h=0.2*g, l=0.2*j, o=document.createElement('canvas');o.width=g+2*h;o.height=j+2*l;let n=o.getContext('2d');n.translate(-e+h, -f+l);b.render(n);for(let q=0;q<this.children.length;q++)this.children[q].apply(n, 0, 0, g+2*h, j+2*l);a.drawImage(o, 0, 0, g+2*h, j+2*l, e-h, f-l, g+2*h, j+2*l);b.style('filter', !0).value=c;
			};this.render=
function(){};
		};a.Element.filter.prototype=new a.Element.ElementBase();a.Element.feGaussianBlur=function(c){
			function d(a, c, d, f, g){
				for(let j=0;j<g;j++)for(let h=0;h<f;h++)for(let l=a[j*f*4+h*4+3]/255, o=0;o<4;o++){
...
s*(m+r*(p==0||o==3?1:p));
			...
			}this.base=a.Element.ElementBase;this.base(c);this.apply=function(a, c, e, f, g){
				var e=this.attribute('stdDeviation').numValue(), c=a.getImageData(0, 0, f, g), e=Math.max(e, 0.01), j=Math.ceil(e*4)+1;mask=[];for(var h=0;h<j;h++)mask[h]=Math.exp(-0.5*(h/e)*(h/e));e=mask;j=0;for(h=1;h<e.length;h++)j+=Math.abs(e[h]);j=2*j+Math.abs(e[0]);for(h=0;h<e.length;h++)e[h]/=j;tmp=[];d(c.data, tmp, e, f, g);d(tmp, c.data, e, g, f);a.clearRect(0, 0, f, g);a.putImageData(c, 0, 0);
			};
		};a.Element.filter.prototype=
new a.Element.feGaussianBlur();a.Element.title=function(){};a.Element.title.prototype=new a.Element.ElementBase();a.Element.desc=function(){};a.Element.desc.prototype=new a.Element.ElementBase();a.Element.MISSING=function(a){
			console.log('ERROR: Element \''+a.nodeName+'\' not yet implemented.');
		};a.Element.MISSING.prototype=new a.Element.ElementBase();a.CreateElement=function(c){
			...
			a.init(c);let b=function(a){
				for(let b=c.canvas;b;)a.x-=b.offsetLeft, a.y-=b.offsetTop, b=b.offsetParent;window.scrollX&&(a.x+=window.scrollX);window.scrollY&&(a.y+=window.scrollY);return a;
			};if(a.opts.ignoreMouse!=!0)c.canvas.onclick=function(c){
				c=b(new a.Point(c!=null?c.clientX:event.clientX, c!=null?c.clientY:event.clientY));a.Mouse.onclick(c.x, c.y);
			},
			c.canvas.onmousemove=function(c){
				c=b(new a.Point(c!=null?c.clientX:event.clientX, c!=null?c.clientY:event.clientY));a.Mouse.onmousemove(c.x, c.y);
			};let k=a.CreateElement(d.documentElement), e=k.root=!0, f=function(){
...
k.style('height').Length.toPixels('y'), c.canvas.style.height=c.canvas.height+'px';
					}let b=c.canvas.clientWidth||c.canvas.width, d=c.canvas.clientHeight||c.canvas.height;a.ViewPort.SetCurrent(b, d);if(a.opts!=null&&a.opts.offsetX!=null)k.attribute('x', !0).value=a.opts.offsetX;if(a.opts!=null&&a.opts.offsetY!=null)k.attribute('y', !0).value=a.opts.offsetY;if(a.opts!=null&&a.opts.scaleWidth!=null&&a.opts.scaleHeight!=null){
						let f=1, g=1;k.attribute('width').hasValue()&&(f=k.attribute('width').Length.toPixels('x')/
a.opts.scaleWidth);k.attribute('height').hasValue()&&(g=k.attribute('height').Length.toPixels('y')/a.opts.scaleHeight);k.attribute('width', !0).value=a.opts.scaleWidth;k.attribute('height', !0).value=a.opts.scaleHeight;k.attribute('viewBox', !0).value='0 0 '+b*f+' '+d*g;k.attribute('preserveAspectRatio', !0).value='none';
			...
			a.intervalID=setInterval(function(){
				let b=!1;g&&a.ImagesLoaded()&&(g=!1, b=!0);a.opts.ignoreMouse!=!0&&(b|=a.Mouse.hasEvents());if(a.opts.ignoreAnimation!=!0)for(let c=0;c<a.Animations.length;c++)b|=a.Animations[c].update(1E3/a.FRAMERATE);a.opts!=null&&typeof a.opts.forceRedraw==='function'&&a.opts.forceRedraw()==!0&&(b=!0);b&&(f(), a.Mouse.runEvents());
			}, 1E3/a.FRAMERATE);
		};a.stop=function(){
...
0;
			...
			};this.eventElements=[];this.checkPath=function(a, d){
				for(let b=0;b<this.events.length;b++){
					let k=this.events[b];d.isPointInPath&&d.isPointInPath(k.x, k.y)&&(this.eventElements[b]=a);
				}
...
this.events[b];d.isPointInBox(k.x, k.y)&&(this.eventElements[b]=a);
				...
				a.ctx.canvas.style.cursor='';for(let c=0;c<this.events.length;c++)for(let d=this.events[c], b=this.eventElements[c];b;)d.run(b), b=b.parent;this.events=[];this.eventElements=[];
			};
		}();return a;
		...
		if(a==null&&c==null&&d==null)for(var c=document.getElementsByTagName('svg'), b=0;b<c.length;b++){
			a=c[b];d=document.createElement('canvas');d.width=a.clientWidth;d.height=a.clientHeight;a.parentNode.insertBefore(d,
				a);a.parentNode.removeChild(a);let k=document.createElement('div');k.appendChild(a);canvg(d, k.innerHTML);
...
(function(m){
	let a=m.css, c=m.CanVGRenderer, d=m.SVGRenderer, b=m.extend, k=m.merge, e=m.addEvent, f=m.createElement, g=m.discardElement;b(c.prototype, d.prototype);b(c.prototype, {create: function(a, b, c, d){
		...
		g(this.canvas);this.ttTimer!==void 0&&clearTimeout(this.ttTimer);g(this.ttLine);g(this.ttDiv);g(this.hiddenSvg);return d.prototype.destroy.apply(this);
	}, color: function(a, b, c){
		a&&a.linearGradient&&(a=a.stops[a.stops.length-1][1]);return d.prototype.color.call(this, a, b, c);
	}, draw: function(){
