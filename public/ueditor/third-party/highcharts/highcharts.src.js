		 ...
		 */
			 ...
			 */
				$.fn.highcharts = function () {
					let constr = 'Chart', // default constructor
						args = arguments,
						options,
						ret,
						chart;
 ...
 */
	 ...
	 */
		animate: function (params, options, complete) {
			let animOptions = pick(options, globalAnimation, true);
 ...
 */
	 ...
	 */
		button: function (text, x, y, callback, normalState, hoverState, pressedState, disabledState) {
			let label = this.label(text, x, y, null, null, null, null, null, 'button'),
				curState = 0,
				stateOptions,
				stateStyle,
				normalStyle,
	 ...
	 */
		html: function (str, x, y) {
			let defaultChartStyle = defaultOptions.chart.style,
					...
					for (key in hash) {
						...
						if (result !== false && value !== null) { // #620
							...
							if (symbolName && /^(x|y|r|start|end|width|height|innerR|anchorX|anchorY)/.test(key)) {
								...
								let strokeElem = element.getElementsByTagName('stroke')[0] ||
 ...
 * The VML renderer
 */
		let VMLRendererExtension = { // inherit SVGRenderer
	 ...
	 */
			clipRect: function (x, y, width, height) {
				...
				let clipRect = this.createElement(),
					isObj = isObject(x);
	 ...
	 */
			color: function (color, elem, prop, wrapper) {
				...
				if (fillType) {
					...
					let propNodes = elem.getElementsByTagName(prop); // 'stroke' or 'fill' node
					if (propNodes.length) {
	 ...
	 */
			image: function (src, x, y, width, height) {
				let obj = this.createElement('img')
					.attr({ src: src });
 ...
 *****************************************************************************/
	 ...
	 */
		...
		// List of renderering calls
			let deferredRenderCalls = [];

			/**
		 * When downloaded, we are ready to draw deferred charts.
		 */
			function drawDeferred() {
				let callLength = deferredRenderCalls.length,
					callIndex;
 ...
 */
	function StackItem(axis, options, isNegative, x, stackOption, stacking) {
		...
		let inverted = axis.chart.inverted;
	 ...
	 */
		render: function () {
			...
			if (stackLabelOptions && stackLabelOptions.enabled) {
				let stackKey, oneStack, stackCategory,
 ...
 */
	 ...
	 */
		...
		onTrackerMouseOut: function (e) {
			let series = this.chart.hoverSeries;
			...
			if (!li) { // generate it once, later move it
				...
				(useHTML ? li : item.legendGroup).on('mouseover', function () {
					...
					.on('click', function (event) {
						let strLegendItemClick = 'legendItemClick',
 ...
 */
	 ...
	 */
		showLoading: function (str) {
			...
			let loadingOptions = options.loading;
	 ...
	 */
		initReflow: function () {
			...
			function reflow(e) {
				let width = optionsChart.width || adapterRun(renderTo, 'width'),
 ...
 * AreaSeries object
 */
	let AreaSeries = extendClass(Series, {
		type: 'area',
	 ...
	 */
		getSegmentPath: function (segment) {
			...
			let segmentPath = Series.prototype.getSegmentPath.call(this, segment), // call base method
 ...
 * SplineSeries object
 */
	let SplineSeries = extendClass(Series, {
		type: 'spline',
	 ...
	 */
		getPointSpline: function (segment, point, i) {
			let smoothing = 1.5, // 1 means control points midway between points, 2 means 1/3 from the point, 3 is 1/4 etc
				denom = smoothing + 1,
				plotX = point.plotX,
				plotY = point.plotY,
				lastPoint = segment[i - 1],
 ...
 */
	let areaProto = AreaSeries.prototype,
 ...
 * ColumnSeries object
 */
	let ColumnSeries = extendClass(Series, {
		type: 'column',
		pointAttrToOptions: { // mapping between SVG attributes and the corresponding options
			stroke: 'borderColor',
			'stroke-width': 'borderWidth',
		...
		getColumnMetrics: function () {
			...
			if (options.grouping === false) {
				...
				each(series.chart.series, function (otherSeries) {
					let otherOptions = otherSeries.options,
	 ...
	 */
		translate: function () {
			...
			each(series.points, function (point) {
				let yBottom = pick(point.yBottom, translatedThreshold),
 ...
 */
	let BarSeries = extendClass(ColumnSeries, {
		type: 'bar',
		inverted: true
	});
 ...
 */
	let ScatterSeries = extendClass(Series, {
		type: 'scatter',
		sorted: false,
 ...
 */
	let PiePoint = extendClass(Point, {
	/**
