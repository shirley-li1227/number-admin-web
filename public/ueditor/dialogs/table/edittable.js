...
(function () {
	...
	let editTable = function () {
		me = this;
		me.init();
	};
	editTable.prototype = {
		init: function () {
			let colorPiker = new UE.ui.ColorPicker({
					editor: editor
				}),
			...
			let enablesortState = editor.queryCommandState('enablesort'),
		...
		captionHanler: function () {
			let example = $G('J_example');
			if (caption.checked) {
				let row = document.createElement('caption');
		...
		autoSizePageHanler: function () {
			let example = $G('J_example');
			let tds = example.getElementsByTagName(example, 'td');
		...
		updateSortSpan: function(){
			...
			let spans = domUtils.getElementsByTagName(example, 'span');
		...
		getColor: function () {
			let start = editor.selection.getStart(), color,
