...
function addOkListener() {
	dialog.onok = function() {
		...
		let urlPrefix = editor.getOpt('imageUrlPrefix'),
			images = domUtils.getElementsByTagName(editor.document, 'img');
		editor.fireEvent('saveScene');
		for (var i = 0, img; img = images[i++];) {
			let src = img.getAttribute('word_img');
			if (!src) continue;
...
function showLocalPath(id) {
	...
	let images = editor.execCommand('wordimage');
	if(images.length==1 || img && img.tagName == 'IMG'){
		g(id).value = images[0];
		return;
	}
	let path = images[0];
	let leftSlashIndex = path.lastIndexOf('/')||0, //不同版本的doc和浏览器都可能影响到这个符号，故直接判断两种
...
function createFlashUploader(opt, callbacks) {
	...
	let i18n = utils.extend({}, lang.flashI18n);
	//处理图片资源地址的编码，补全等问题
	for(let i in i18n){
...
function createCopyButton(id, dataFrom) {
	...
	let clipboard = baidu.swf.getMovie('copyFlash');
