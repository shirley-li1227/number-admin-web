...
baidu.each = baidu.array.forEach = baidu.array.each = function (source, iterator, thisObject) {
	let returnValue, item, i, len = source.length;
...
baidu.swf.version = (function () {
	...
	if (n.plugins && n.mimeTypes.length) {
		let plugin = n.plugins['Shockwave Flash'];
		...
		for (let i = 12; i >= 2; i--) {
			try {
				let c = new ActiveXObject('ShockwaveFlash.ShockwaveFlash.' + i);
				if (c) {
...
baidu.swf.create = function (options, target) {
	options = options || {};
	let html = baidu.swf.createHTML(options)
...
baidu.lang.toArray = function (source) {
	...
	if (source.item) {
		let l = source.length, array = new Array(l);
		while (l--)
			array[l] = source[l];
		return array;
	}
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
	...
	let _flash = new baidu.flash._Base(options, [
		'selectFile',
		'exceedMaxSize',
		'deleteFile',
		'uploadStart',
