...
(function(){
	...
	/* 初始化tab标签 */
	function initTabs(){
		let tabs = $G('tabHeads').children;
		for (let i = 0; i < tabs.length; i++) {
			domUtils.on(tabs[i], 'click', function (e) {
				let j, bodyId, target = e.target || e.srcElement;
	...
	function initVideo(){
		...
		//编辑视频时初始化相关信息
		(function(){
			let img = editor.selection.getRange().getClosedNode(), url;
			if(img && img.className){
				let hasFakedClass = (img.className == 'edui-faked-video'),
					hasUploadClass = img.className.indexOf('edui-upload-video')!=-1;
				if(hasFakedClass || hasUploadClass) {
					$G('videoUrl').value = url = img.getAttribute('_url');
					$G('videoWidth').value = img.width;
					$G('videoHeight').value = img.height;
					let align = domUtils.getComputedStyle(img, 'float'),
	...
	function addOkListener(){
		dialog.onok = function(){
			$G('preview').innerHTML = '';
			let currentTab = findFocus('tabHeads', 'tabSrc');
			switch(currentTab){
				case 'video':
					return insertSingle();
					break;
	...
	function updateAlignButton( align ) {
		let aligns = $G( 'videoFloat' ).children;
	...
	function insertSearch(id){
		let imgs = domUtils.getElementsByTagName($G(id), 'img'),
			videoObjs=[];
	...
	function switchSelect( selectParentId ) {
		let selects = $G( selectParentId ).children;
	...
	function insertUpload(){
		...
		for(let key in uploadVideoList) {
			let file = uploadVideoList[key];
			videoObjs.push({
		...
		let count = uploadFile.getQueueCount();
		if (count) {
