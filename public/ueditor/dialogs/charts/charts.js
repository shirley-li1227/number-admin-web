...
function initChartsTypeView () {
	...
	for ( let i = 0, len = chartsConfig.length; i<len; i++ ) {
...
function initEvent () {
	...
	$( '#scrollBed' ).delegate( '.view-box', 'click', function (e) {
		...
		let index = $( this ).attr( 'data-chart-type' );
...
function getSeriesAndCategories () {
	...
	if ( getTableDataFormat() === '-1' ) {
		...
		for ( var i = 0, len = tableData.length; i < len; i++ ) {
			...
			for ( let j = 0, jlen = tableData[ i ].length; j < jlen; j++ ) {
...
function getSeriesForPieChart () {
	...
	for ( let j = 1, jlen = tableData[ 0 ].length; j < jlen; j++ ) {

		let title = tableData[ 0 ][ j ],
			val = tableData[ 1 ][ j ];
