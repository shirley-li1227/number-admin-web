...
(function () {
	...
	function setTabFocus(id) {
		if(!id) return;
		let i, bodyId, tabs = $G('tabhead').children;
	...
	function initButtons() {
		...
		dialog.onok = function () {
			let remote = false, list = [], id, tabs = $G('tabhead').children;
			for (let i = 0; i < tabs.length; i++) {
	...
	function setAlign(align){
		align = align || 'none';
		let aligns = $G('alignIcon').children;
	...
	RemoteImage.prototype = {
		...
		initEvents: function () {
			...
			domUtils.on($G('width'), 'keyup', function(){
				updatePreview();
				if(locker.checked) {
					let proportion =locker.getAttribute('data-proportion');
			...
			domUtils.on($G('lock'), 'change', function(){
				let proportion = parseInt($G('width').value) /parseInt($G('height').value);
		...
		setImage: function(img){
			...
			let wordImgFlag = img.getAttribute('word_img'),
	...
	UploadImage.prototype = {
		...
		getQueueCount: function () {
			let file, i, status, readyFile = 0, files = this.uploader.getFiles();
	...
	OnlineImage.prototype = {
		...
		initEvents: function () {
			...
			$('#online').on('mouseenter', 'li', function() {
				$(this).find('.bn-panel').stop().animate({height: 30});
			});

			$('#online').on('mouseleave', 'li', function() {
				$(this).find('.bn-panel').stop().animate({height: 0});
			});

			$('#online').on('click', '.bn-del', function() {
				_this.delOnlineImage($(this));
			});
		},
		/* 初始化第一次的数据 */
		initData: function () {
		...
		getImageData: function () {
			...
			if(!_this.listEnd && !this.isLoadingData) {
				...
				let url = editor.getActionUrl(editor.getOpt('imageManagerActionName')),
					...
					'onsuccess': function (r) {
						try {
							...
							if(r.responseText.indexOf('ue_separate_ue') != -1) {
								let list = r.responseText.split(r.responseText);
								_this.pushData(list);
		...
		delOnlineImage: function ($delBtn){
			let _this = this;
			let imageUrl = $delBtn.attr('url');
			if(!imageUrl) return;
			let url = editor.getActionUrl(editor.getOpt('imageManagerDeleteActionName'));
			console.log(url);
			ajax.request(url + '?imgpath='+imageUrl, {
				'timeout': 3000,
				'dataType': 'json',
				'method': 'get',
				'onsuccess': function (r) {
		...
		pushData: function (list) {
			let i, item, img, icon, _this = this,
			...
			for (i = 0; i < list.length; i++) {
				if(list[i] && list[i].url) {
					...
					let bnPanel = document.createElement('div');
					domUtils.addClass(bnPanel, 'bn-panel');
					let bnDel = document.createElement('span');
					bnDel.setAttribute('url', list[i].url);
					domUtils.addClass(bnDel, 'bn-del');

					bnPanel.appendChild(bnDel);
					item.appendChild(img);
					item.appendChild(icon);
					item.appendChild(bnPanel);
		...
		getInsertList: function () {
			let i, lis = this.list.children, list = [], align = getAlign();
			for (i = 0; i < lis.length; i++) {
				if (domUtils.hasClass(lis[i], 'selected')) {
					let img = lis[i].firstChild,
	...
	SearchImage.prototype = {
		...
		setList: function (list) {
			let i, item, p, img, link, _this = this,
