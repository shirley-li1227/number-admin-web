 ...
 */
let scrawl = function (options) {
	options && this.initOptions(options);
};
(function () {
	let canvas = $G('J_brushBoard'),
	...
	scrawl.prototype = {
		...
		_clearSelection: function () {
			let list = ['J_operateBar', 'J_colorBar', 'J_brushBar', 'J_eraserBar', 'J_picBoard'];
		...
		_originalColorSelect: function (title) {
			let colorList = $G('J_colorList').getElementsByTagName('td');
		...
		_originalBrushSelect: function (text) {
			let brushList = $G('J_brushBar').children;
			for (var i = 0, ele; ele = brushList[i++];) {
				if (ele.tagName.toLowerCase() == 'a') {
					let size = browser.ie ? ele.innerText : ele.text;
		...
		_addBESelect: function (target) {
			let brushList = $G('J_brushBar').children;
			let eraserList = $G('J_eraserBar').children;
...
function ue_callback(url, state) {
	...
	function scale(img, max, oWidth, oHeight) {
		let width = 0, height = 0, percent, ow = img.width || oWidth, oh = img.height || oHeight;
		if (ow > max || oh > max) {
			if (ow >= oh) {
				if (width = ow - max) {
...
function exec(scrawlObj) {
	if (scrawlObj.isScrawl) {
		...
		let base64 = scrawlObj.getCanvasData();
		if (!!base64) {
			let options = {
				timeout: 100000,
			...
			let actionUrl = editor.getActionUrl(editor.getOpt('scrawlActionName')),
